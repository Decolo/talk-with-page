...
class SidePanelController {
    ...
    if (needsFullPageData && tab?.id) {
      try {
        ...
        if (pageData && pageData.content) {
          ...
          console.log('[Side Panel] Sending full page context:', {
            url: currentUrl,
          ...
          console.warn('[Side Panel] Page data is empty, sending minimal context');
        }
      } catch (error) {
        console.error('[Side Panel] Failed to get page data:', error);
        this.addMessage('status', 'Note: Could not load page content. The page may not allow content scripts.');
        // Continue with minimal context
      }
    } else {
      console.log('[Side Panel] Sending minimal context (reusing stored context)');
    }
  ...
  private async getFullPageData(tabId: number): Promise<{
    ...
    try {
      ...
      console.log('[Side Panel] Content script not responding, injecting programmatically...');
      ...
      try {
        ...
        console.error('[Side Panel] Failed to inject content script:', injectionError);
